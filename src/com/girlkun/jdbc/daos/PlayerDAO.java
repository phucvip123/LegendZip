...
public class PlayerDAO {
        ...
        try {
           ...
           List<Boolean> list = new ArrayList<>(Arrays.asList(new Boolean[Manager.ACHIEVEMENTS.size()]));
            ...
            GirlkunDB.executeUpdate("insert into player"
                    ...
                    + "data_black_ball, data_side_task, data_card, bill_data, info_phoban, data_may_do, info_achievement,data_duoi_khi) "
                    ...
                    itemTime, itemTimeSC, task, mabuEgg, charms, skills, skillsShortcut, petData, dataBlackBall, dataSideTask, data_card, bill_data, info_phoban, itemMayDo, info_achive,dkhi);
        ...
        if (player.iDMark.isLoadedAllDataPlayer()) {
            ...
            try {
                ...
                String query = " update player set info_achievement =?, data_may_do = ?, khi_gas =?, data_item_time_sieu_cap = ?,head = ?, have_tennis_space_ship = ?,"
                        ...
                        + " skills_shortcut = ?, pointPvp=?, NguHanhSonPoint=?,data_card=?,bill_data =? ,info_phoban =? ,data_duoi_khi =? where id = ?";
    ...
    public static boolean activePlayer(Player player){
        ...
        try (Connection con = GirlkunDB.getConnection();) {
            ps = con.prepareStatement("update account set active = 1 where name = '"+player.name+"'");
            ps.executeUpdate();
            ps.close();
        } catch (Exception e) {
            Logger.logException(PlayerDAO.class, e, "Lỗi kích hoạt " + player.name);
            return false;
        } finally {
        }
        return true;

...
//                    }
    ...
    private void phanradothanlinh(Player player) {
        if (player.combineNew.itemsCombine.size() == 1) {
            ...
            this.baHatMit.npcChat(player, "Con đã nhận được " + couponAdd + " điểm và 1 điểm chiến lực");
            InventoryServiceNew.gI().subQuantityItemsBag(player, item, 1);
            player.combineNew.itemsCombine.clear();
            InventoryServiceNew.gI().sendItemBags(player);
            Service.gI().sendMoney(player);
            Item it2 = new Item((short)1144);
            it2.quantity = 1;
            InventoryServiceNew.gI().addItemBag(player, it2);
            InventoryServiceNew.gI().sendItemBags(player);
            reOpenItemCombine(player);
        }
    }
...
//    }
    ...
    private int getRubydnangbt(int lvbt) {
        return RUBY_BONG_TAI2 + 500 * lvbt;
    }

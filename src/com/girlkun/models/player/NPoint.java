    ...
    public double getDameAttack(boolean isAttackMob) {
        ...
        switch (skillSelect.template.id) {
            ...
            case Skill.DICH_CHUYEN_TUC_THOI:
                dameAttack *= 1;
                dameAttack = dameAttack * (1 + Util.nextInt(-1,1))/100;
                return dameAttack;
...
//        }
        ...
        dameAttack = dameAttack + dameAttack*Util.nextInt(-2,2)/100;
        if (player.isPl()) {
        ...
        if(dameAttack>=Integer.MAX_VALUE){
                DecimalFormat decimalFormat = new DecimalFormat("#,###");
                String dame = decimalFormat.format(dameAttack);
                dame = dame.replaceAll(",",".");           
                Service.gI().sendThongBao(player, "|1|Tấn công thật của bạn là: \n" + "|7|"+ dame);
        }
        return dameAttack;
    }
...
//            tiemNang *= Manager.RATE_EXP_SERVER;
    ...
    public void increasePoint(byte type, short point) {
        ...
        if (type == 0) {
            ...
            tiemNangUse = Util.maxLong(point * (2 * (this.hpg + 1000) + pointHp - 20) / 2);
        ...
        if (type == 1) {
            ...
            tiemNangUse = Util.maxLong(point * (2 * (this.mpg + 1000) + pointMp - 20) / 2);
        ...
        if (type == 2) {
            tiemNangUse = Util.maxLong(point * (2 * this.dameg + point - 1) / 2 * 100);
        ...
        if (type == 3) {
            tiemNangUse = Util.maxLong(2 * (this.defg + 5) / 2 * 100000);

...
//                        System.out.println("Cộng chỉ số cấp 2");
        ...
        boolean foundItem1128 = false;
        for (Item item : this.player.inventory.itemsBag) {
            if (item.isNotNullItem() && (item.template.id == 1128)) {
                if (!foundItem1128 && this.player.fusion.typeFusion == ConstPlayer.HOP_THE_PORATA3) {
                    foundItem1128 = true;
                    for (Item.ItemOption io : item.itemOptions) {
                        switch (io.optionTemplate.id) {
                            case 80: //HP+#%/30s
...
//                int dameX44 =  player.inventory.getParam(player.inventory.itemsBody.get(5), 159);
//                if (dameX44 > 0) {
//                dameAttack *= dameX44;
//                }
                percentDameSkill = skillSelect.damage;
...
//                int dameX =  player.inventory.getParam(player.inventory.itemsBody.get(5), 159);
//                if (dameX > 0) {
//                dameAttack *= dameX;
//                }
    ...
    public long calSucManhTiemNang(long tiemNang) {
        if (power < getPowerLimit()) {
            ...
            if (this.player.isPet) {
                ...
                if(((Pet) this.player).nPoint.PercentTnsmDeTu > 0){
                    tiemNang += (tiemNang * ((Pet) this.player).nPoint.PercentTnsmDeTu)/1;
                }
            }

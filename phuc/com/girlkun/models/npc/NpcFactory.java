...
import com.arriety.models.map.bdkb.BanDo;
import com.arriety.models.map.bdkb.BanDoService;
...
//                                        
                                ...
                                case 2:
                                    ...
                                    if (player.clan.getMembers().size() < BanDo.N_PLAYER_CLAN) {
                                        this.createOtherMenu(player, ConstNpc.IGNORE_MENU,
                                                "Bang hội phải có ít nhất 5 thành viên mới có thể mở", "Đóng");
                                        return;
                                    }
                                    if (player.clan.haveGoneBDKB) {
                                        createOtherMenu(player, ConstNpc.IGNORE_MENU,
                                                "Bang hội của ngươi đã đi Bản Đồ lúc " + TimeUtil.formatTime(player.clan.lastTimeOpenBDKB, "HH:mm:ss") + " hôm nay. Người mở\n"
                                                + "(" + player.clan.playerOpenBDKB + "). Hẹn ngươi quay lại vào ngày mai", "OK", "Hướng\ndẫn\nthêm");
                                        return;
                                    }
                                    if (player.clan.bando != null) {
                                        createOtherMenu(player, ConstNpc.MENU_OPENED_DBKB,
                                                "Bang hội của cậu đang tham gia Bản đồ kho báu cấp độ " + player.clan.bando.level + "\n"
                                                + "Thời gian còn lại là "
                                                + TimeUtil.getSecondLeft(player.clan.bando.getLastTimeOpen(), BanDo.TIME_BAN_DO / 1000)
                ...
                if (canOpenNpc(player)) {
                    if (!TaskService.gI().checkDoneTaskTalkNpc(player, this)) {
                        this.createOtherMenu(player, ConstNpc.BASE_MENU,
                                ...
                                "Đổi mật khẩu", "Nhận ngọc xanh", "Nhận vàng", "Nhận đệ tử", "Cách kiếm vàng", "Kích hoạt\n Tài khoản", "GiftCode");
                    }
                }
            }
...
//                                      
                        ...
                        if (select == 1) {
                            Service.gI().showListTop(player, Manager.topNHS,(byte)0);
                        ...
                        case ConstNpc.BASE_MENU:
                            switch (select) {
                                case 0:
                                    ...
                                    break;
                                case 1:
                                     Service.gI().showListTop(player, Manager.topKhiGas,(byte)0);
                                    break;
                                case 2:
                    ...
                    switch (this.mapId) {
                        case 48:
                            switch (player.iDMark.getIndexMenu()) {
                                case ConstNpc.BASE_MENU:
                                    if (select == 0) {
                                        this.npcChat(player, "Hello sờ ly ly");
                    ...
                    switch (this.mapId) {
                        case 5:
                            switch (player.iDMark.getIndexMenu()) {
                                case ConstNpc.BASE_MENU:
                                    if (select == 0) {
                                        Service.gI().showListTop(player, Manager.topSM,(byte)0);
                                        break;
                                    }
                                    if (select == 1) {
                                        Service.gI().showListTop(player, Manager.topNV,(byte)0);
                                        break;
                                    }
                                    if (select == 2) {
                                        Service.gI().showListTop(player, Manager.topPVP,(byte)0);
                                        break;
                                    }
                                    if (select == 3) {
                                        Service.gI().showListTop(player, Manager.topNHS,(byte)0);
                    ...
                    case ConstNpc.MENU_EVENT:
                        switch (select) {
                            case 0:
                                ...
                                break;
                            case 1:
                                Service.gI().showListTop(player, Manager.topSK,(byte)0);
                                break;
                            case 2:

...
//                msg.writer().writeByte(0);
    ...
    public List<ItemKyGui> getItemCanKiGui(Player pl) {
        ...
        pl.inventory.itemsBag.stream().filter((it) -> (it.isNotNullItem() && (it.template.type == 12 || it.template.type == 14 || it.template.type == 33 || it.template.type == 29 || checkoptionkygui(it)))).forEachOrdered((it) -> {
    ...
    public boolean checkoptionkygui (Item itemkg){
        for (Item.ItemOption io : itemkg.itemOptions) {
            if (io.optionTemplate.id == 86 || io.optionTemplate.id == 87) {
                return true;
            }
        }
        return false;
